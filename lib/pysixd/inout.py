...
def load_scene_camera(path):
    ...
    for im_id in scene_camera.keys():
        if "cam_K" in scene_camera[im_id].keys():
            scene_camera[im_id]["cam_K"] = np.array(scene_camera[im_id]["cam_K"], np.float_).reshape((3, 3))
        if "cam_R_w2c" in scene_camera[im_id].keys():
            scene_camera[im_id]["cam_R_w2c"] = np.array(scene_camera[im_id]["cam_R_w2c"], np.float_).reshape((3, 3))
        if "cam_t_w2c" in scene_camera[im_id].keys():
            scene_camera[im_id]["cam_t_w2c"] = np.array(scene_camera[im_id]["cam_t_w2c"], np.float_).reshape((3, 1))
    return scene_camera
    ...
    for im_id, im_gt in scene_gt.items():
        for gt in im_gt:
            if "cam_R_m2c" in gt.keys():
                gt["cam_R_m2c"] = np.array(gt["cam_R_m2c"], np.float_).reshape((3, 3))
            if "cam_t_m2c" in gt.keys():
                gt["cam_t_m2c"] = np.array(gt["cam_t_m2c"], np.float_).reshape((3, 1))
    return scene_gt
            ...
            for line in f:
                ...
                if line_id == 1 and header in line:
                    ...
                    result = {
                        ...
                        "obj_id": int(elems[2]),
                        "score": float(elems[3]),
                        "R": np.array(list(map(float, elems[4].split())), np.float_).reshape((3, 3)),
                        "t": np.array(list(map(float, elems[5].split())), np.float_).reshape((3, 1)),
                        "time": float(elems[6]),
                    }
...
def load_ply(path, vertex_scale=1.0):
    ...
    if {"texture_u", "texture_v"}.issubset(set(pt_props_names)):
        is_texture_pt = True
        model["texture_uv"] = np.zeros((n_pts, 2), np.float_)

    is_texture_face = False
    if {"texcoord"}.issubset(set(face_props_names)):
        is_texture_face = True
        model["texture_uv_face"] = np.zeros((n_faces, 6), np.float_)
...
def save_ply_float_color(
    path,
    pts,
    pts_colors=np.array([]),
    pts_normals=np.array([]),
    faces=np.array([]),
):
    # https://github.com/thodan/sixd_toolkit/blob/master/pysixd/inout.py
    """Saves a 3D mesh model to a PLY file.
...
if __name__ == "__main__":
    ...
    print(
        "train",
        model_train["pts"].shape,
        model_train["pts"].min(0),
        model_train["pts"].max(0),
    )
