...
#     pts_im = np.round(pts_im).astype(np.int_)
...
def paste_emb_to_im_batch(embs, bboxes_xyxy, height=480, width=640):
    ...
    for i in range(num):
        ...
        if src is None:
            tgt_embs[i] = None
            continue
        x1, y1, x2, y2 = bboxes_xyxy[i, :4].astype(np.int_)
...
def ply_vtx_color_expand(model):
    ...
    ptsExpand = np.array(ptsExpand, dtype=np.float_)
    colorsExpand = np.array(colorsExpand, dtype=np.float_)
...
def test_draw_3d_bbox():
    ...
    gt_dict = mmengine.load(os.path.join(cur_dir, "../../datasets/BOP_DATASETS/lm/test/000001/scene_gt.json"))
