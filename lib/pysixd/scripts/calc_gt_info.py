...
if p["vis_visibility_masks"]:
    from lib.pysixd import visualization
    ...
    for im_counter, im_id in enumerate(im_ids):
        ...
        for gt_id, gt in enumerate(scene_gt[im_id]):
            ...
            depth_gt_large = ren.render_object(
                gt["obj_id"], gt["cam_R_m2c"], gt["cam_t_m2c"], fx, fy, cx + ren_cx_offset, cy + ren_cy_offset
            )["depth"]
            depth_gt = depth_gt_large[
                ren_cy_offset : (ren_cy_offset + im_height), ren_cx_offset : (ren_cx_offset + im_width)
            ]
            ...
            visib_gt = visibility.estimate_visib_mask_gt(dist_im, dist_gt, p["delta"], visib_mode="bop19")
            ...
            if p["vis_visibility_masks"]:
                ...
                visib_gt_vis = visib_gt.astype(np.float_)
