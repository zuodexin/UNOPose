...
for scene_id in scene_ids:
    ...
    for im_id in scene_gt.keys():
        for gt_id, im_gt in enumerate(scene_gt[im_id]):
            ...
            # tilt
            euler = [r / np.pi * 180 for r in mat2euler(im_gt["cam_R_m2c"], axes="szxz")]
            tilts.append(euler[2])
...
misc.log("Min elev: {}".format(np.min(tilts)))
misc.log("Max elev: {}".format(np.max(tilts)))
misc.log("Mean elev: {}".format(np.mean(tilts)))
