...
from lib.pysixd import visibility
...
for scene_id in scene_ids:
    ...
    mask_dir_path = os.path.dirname(dp_split["mask_tpath"].format(scene_id=scene_id, im_id=0, gt_id=0))
    misc.ensure_dir(mask_dir_path)

    mask_visib_dir_path = os.path.dirname(dp_split["mask_visib_tpath"].format(scene_id=scene_id, im_id=0, gt_id=0))
    misc.ensure_dir(mask_visib_dir_path)

    # Initialize a renderer.
    misc.log("Initializing renderer...")
    width, height = dp_split["im_size"]
    ren = renderer.create_renderer(width, height, renderer_type=p["renderer_type"], mode="depth")
    ...
    for im_id in im_ids:
        ...
        for gt_id, gt in enumerate(scene_gt[im_id]):
            ...
            depth_gt = ren.render_object(gt["obj_id"], gt["cam_R_m2c"], gt["cam_t_m2c"], fx, fy, cx, cy)["depth"]
            ...
            mask_visib = visibility.estimate_visib_mask_gt(dist_im, dist_gt, p["delta"], visib_mode="bop19")

            # Save the calculated masks.
            mask_path = dp_split["mask_tpath"].format(scene_id=scene_id, im_id=im_id, gt_id=gt_id)
            inout.save_im(mask_path, 255 * mask.astype(np.uint8))

            mask_visib_path = dp_split["mask_visib_tpath"].format(scene_id=scene_id, im_id=im_id, gt_id=gt_id)
