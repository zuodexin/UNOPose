...
class UNOPose(nn.Module):
    def __init__(self, cfg):
        ...
        self.B_own = True  # 控制是否使用我们传入的csv数据。设置为False即为原先
    ...
    def forward(self, end_points, B_R = None, B_t = None):
        ...
        # 将我们的csv数据(R和t)覆盖进end_points
        if self.B_own == True:
            end_points["init_R"] = B_R.to(radius.device)
            t_normalized = B_t.to(radius.device) / (radius.reshape(-1, 1) + 1e-6)
            end_points["init_t"] = t_normalized.to(radius.device)
            end_points["init_pose_score"] = torch.ones_like(end_points["init_pose_score"]) # score暂时设置为1.0
