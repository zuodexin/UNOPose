...
import core.unopose.model.pointnet2.pytorch_utils as pt_utils
import sys
...
try:
    import core.unopose.model.pointnet2._ext as _ext
except ImportError:
...
class LRF_batch(nn.Module):
    def __init__(self, eps=1e-10, r_lrf=0.1):
        super(LRF_batch, self).__init__()

        self.eps = eps
        self.r_lrf = r_lrf

    def forward(self, xyz, xyz_group):
        B, N, c, M = xyz_group.size()
        xyz_group = xyz_group.contiguous()  # dim = B, N, 3, M
        xyz = xyz.unsqueeze(3).contiguous()  # dim = B, N, 3, 1
        ...
        zp = (_sign * v[..., -1]).unsqueeze(2)  # B, N, 1, 3

        # xp
        x *= -1  # p->pi = pi - p
        norm = (zp @ x).transpose(2, 3)
        proj = norm * zp
        ...
        lrf = torch.cat((xp.unsqueeze(3), yp.unsqueeze(3), zp.transpose(2, 3)), dim=3)

        _out_xyz = (xyz_group - xyz) / self.r_lrf
        out_xyz = lrf.transpose(2, 3) @ _out_xyz

        return out_xyz


class QueryAndLRFGroup(nn.Module):
    r"""
    Groups with a ball query of radius
    ...
    def __init__(
        ...
        super(QueryAndLRFGroup, self).__init__()
        self.radius, self.nsample, self.use_xyz = radius, nsample, use_xyz
    ...
    def forward(self, xyz, new_xyz, features=None):
        # type: (QueryAndLRFGroup, torch.Tensor. torch.Tensor, torch.Tensor) -> Tuple[Torch.Tensor]
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
        ...
        new_features : torch.Tensor
            (B, 3 + C, npoint, nsample) tensor
        """
        # NOTE: if xyz == new xyz, the raw point would be searched!
        idx = ball_query(self.radius, self.nsample, xyz, new_xyz)
        ...
        if features is not None:
            grouped_features = grouping_operation(features, idx)
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, lrf_features], dim=1)  # (B, 3 + 3, npoint, nsample)
            else:
