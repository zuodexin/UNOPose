    ...
    def _get_template(self, path, nView):
        if nView > 0:
            ...
            for v in range(nView):
                ...
                # saved in minimeter?  512x512x3
                xyz = np.load(xyz_path).astype(np.float32) / 1000.0  # * self.diameter # / 1000.0
                mask = load_im(mask_path, "unchanged").astype(np.int32) == 255
...
def load_obj(
    ...
    models_info = load_json(os.path.join(model_path, "models_info.json"))
    ...
    if "symmetries_continuous" in models_info[str(obj_id)]:
        ...
        symmetry_flag = 0
    return Obj(obj_id, mesh, model_points, diameter, symmetry_flag, template_path, n_template_view)


def load_objs(
    model_path="models", template_path="render_imgs", sample_num=512, n_template_view=0, show_progressbar=True
):
    objs = []
    obj_ids = sorted([int(p.split("/")[-1][4:10]) for p in glob.glob(os.path.join(model_path, "*.ply"))])

    if n_template_view > 0:
        template_paths = sorted(glob.glob(os.path.join(template_path, "*")))  # /path/to/obj_000001 ...
        assert len(template_paths) == len(obj_ids), "{} template_paths, {} obj_ids".format(
            len(template_paths), len(obj_ids)
        )
    else:
    ...
    for obj_id in tqdm(obj_ids, "loading objects") if show_progressbar else obj_ids:
        # NOTE: num template paths should be equal to num objs
        # and the sorted order should be the same as the order of obj ids
        # (make sure that model path and template path have the same name pattern)
        objs.append(load_obj(model_path, obj_id, sample_num, template_paths[cnt], n_template_view))
        cnt += 1
    return objs, obj_ids
