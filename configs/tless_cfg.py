...
from core.unopose.provider.pfoneref_bop_test_dataset_v2 import (
    BOPTestsetPoseFreeOneRefv2,
)

from core.unopose.provider.build_data_loader import (
    build_train_loader,
    build_test_loader,
)
...
model = L(UNOPose)(
    cfg=dict(
        ...
        feature_extraction=dict(
            ...
            pretrained=True,
            vit_ckpt=osp.join(
                PROJ_ROOT, "checkpoints/timm_vit_base_patch14_reg4_dinov2_lvd142m.pth"
            ),
            freeze_vit=True,
        ),
        geo_embedding=dict(
        ...
        test=L(build_test_loader)(
            dataset=L(BOPTestsetPoseFreeOneRefv2)(
                cfg=dict(
                    ...
                    ref_targets_name="test_ref_targets_firstview.json",  #
                    img_size=224,
                    n_sample_observed_point=2048,
                ...
                eval_dataset_name="tless",
                detetion_path=osp.join(
                    PROJ_ROOT,
                    "datasets/segmentation/gt_segmentation_tless_test.json",  #
                ),
            ),
            num_workers=16,
        ),
