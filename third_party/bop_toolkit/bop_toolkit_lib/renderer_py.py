...
class RendererPython(renderer.Renderer):
    ...
    def __init__(self, width, height, mode="rgb+depth", shading="phong", bg_color=(0.0, 0.0, 0.0, 0.0)):
        ...
        rgb_buf = np.zeros((self.height, self.width, 4), np.float32).view(gloo.TextureFloat2D)
        depth_buf = np.zeros((self.height, self.width), np.float32).view(gloo.DepthTexture)
    ...
    def render_object(self, obj_id, R, t, fx, fy, cx, cy):
        ...
        mat_proj = _calc_calib_proj(K, 0, 0, self.width, self.height, clip_near, clip_far)
        ...
        def on_draw(dt):
            ...
            if self.render_depth:
                self.depth = self._draw_depth(curr_obj_id, mat_model, mat_view, mat_proj)
