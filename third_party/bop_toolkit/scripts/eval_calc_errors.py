        ...
        for im_ind, (im_id, im_targets) in enumerate(scene_targets.items()):
            if im_ind % 10 == 0:
                misc.log(
                    ...
                    "im: {}".format(p["error_type"], method, dataset, split_type_str, scene_id, im_ind)
                )
            ...
            for obj_id, target in im_targets.items():
                ...
                for est_id, est in obj_ests_sorted:
                    ...
                    scene_errs.append(
                        {"im_id": im_id, "obj_id": obj_id, "est_id": est_id, "score": est["score"], "errors": errs}
                    )
        ...
        if p["error_type"] == "vsd":
            ...
            for vsd_tau_id, vsd_tau in enumerate(p["vsd_taus"]):
                error_sign = misc.get_error_signature(
                    p["error_type"], p["n_top"], vsd_delta=p["vsd_deltas"][dataset], vsd_tau=vsd_tau
                )
