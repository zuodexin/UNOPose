...
p = {
    ...
    "vis_rgb_tpath": os.path.join("{vis_path}", "{result_name}", "{scene_id:06d}", "{vis_name}.jpg"),
    "vis_depth_diff_tpath": os.path.join("{vis_path}", "{result_name}", "{scene_id:06d}", "{vis_name}_depth_diff.jpg"),
}
################################################################################
...
for result_fname in p["result_filenames"]:
    ...
    ren = renderer.create_renderer(width, height, p["renderer_type"], mode=renderer_mode)
    ...
    for scene_id, scene_ests in ests_org.items():
        ...
        for im_ind, (im_id, im_ests) in enumerate(scene_ests.items()):
            ...
            for obj_id, obj_ests in im_ests.items():
                ...
                obj_ests_sorted = sorted(obj_ests, key=lambda est: est["score"], reverse=True)
                ...
                for est in obj_ests_sorted:
                    ...
                    if p["vis_per_obj_id"]:
                        est["text_info"] = [{"name": "", "val": est["score"], "fmt": ":.2f"}]
                    else:
            ...
            for ests_vis_id, ests_vis in enumerate(im_ests_vis):
                ...
                # Visualization name.
                if p["vis_per_obj_id"]:
                    vis_name = "{im_id:06d}_{obj_id:06d}".format(im_id=im_id, obj_id=im_ests_vis_obj_ids[ests_vis_id])
                else:
                ...
                if p["vis_rgb"]:
                    vis_rgb_path = p["vis_rgb_tpath"].format(
                        vis_path=p["vis_path"], result_name=result_name, scene_id=scene_id, vis_name=vis_name
                    )
