...
p = {
    ...
    "vis_rgb_tpath": os.path.join("{vis_path}", "{dataset}", "{split}", "{scene_id:06d}", "{im_id:06d}.jpg"),
    ...
    for im_counter, im_id in enumerate(im_ids):
            ...
            gt_poses.append(
                {
                    ...
                    "R": gt["cam_R_m2c"],
                    "t": gt["cam_t_m2c"],
                    "text_info": [{"name": "", "val": "{}:{}".format(gt["obj_id"], gt_id), "fmt": ""}],
                }
            )
        ...
        if p["vis_rgb"]:
            vis_rgb_path = p["vis_rgb_tpath"].format(
                vis_path=p["vis_path"], dataset=p["dataset"], split=p["dataset_split"], scene_id=scene_id, im_id=im_id
            )
