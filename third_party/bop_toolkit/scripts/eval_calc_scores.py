...
p = {
    ...
    "error_tpath": os.path.join("{eval_path}", "{error_dir_path}", "errors_{scene_id:06d}.json"),
    # Template of path to the output file with established matches and calculated
    # scores.
    "out_matches_tpath": os.path.join("{eval_path}", "{error_dir_path}", "matches_{score_sign}.json"),
    "out_scores_tpath": os.path.join("{eval_path}", "{error_dir_path}", "scores_{score_sign}.json"),
}
################################################################################
...
for err_type in p["correct_th"]:
    parser.add_argument("--correct_th_" + err_type, default=",".join(map(str, p["correct_th"][err_type])))

parser.add_argument("--normalized_by_diameter", default=",".join(p["normalized_by_diameter"]))
parser.add_argument("--normalized_by_im_width", default=",".join(p["normalized_by_im_width"]))
...
for error_dir_path in p["error_dir_paths"]:
    ...
    misc.log("Calculating score - error: {}, method: {}, dataset: {}.".format(err_type, method, dataset))
    ...
    for scene_id, scene_targets in targets_org.items():
        ...
        for im_id, im_targets in scene_targets.items():
            ...
            if p["visib_gt_min"] >= 0:
                ...
                gt_ids_sorted = sorted(
                    range(len(im_gt)), key=lambda gt_id: im_gt_info[gt_id]["visib_fract"], reverse=True
                )
        ...
        matches += pose_matching.match_poses_scene(
            scene_id, scene_gt_curr, scene_gt_valid, scene_errs, p["correct_th"][err_type], n_top
        )

    # Calculate the performance scores.
    # ----------------------------------------------------------------------------
    # 6D object localization scores (SiSo if n_top = 1).
    scores = score.calc_localization_scores(dp_split["scene_ids"], dp_model["obj_ids"], matches, n_top)
