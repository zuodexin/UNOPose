...
ren = renderer.create_renderer(width, height, p["renderer_type"], mode="rgb", shading="flat")
...
for obj_id in dp_model["obj_ids"]:
    ...
    poses = misc.get_symmetry_transformations(models_info[obj_id], p["max_sym_disc_step"])
    ...
    for pose_id, pose in enumerate(poses):
        for view_id, view in enumerate(p["views"]):
            ...
            vis_rgb_path = p["vis_rgb_tpath"].format(
                vis_path=p["vis_path"], dataset=p["dataset"], obj_id=obj_id, view_id=view_id, pose_id=pose_id
            )
