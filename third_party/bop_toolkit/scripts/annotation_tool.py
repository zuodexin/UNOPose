...
class AppWindow:
    ...
    def _on_layout(self, layout_context):
        ...
        height = min(
            r.height, self._settings_panel.calc_preferred_size(layout_context, gui.Widget.Constraints()).height
        )
    ...
    def _make_point_cloud(self, rgb_img, depth_img, cam_K):
        ...
        intrinsic = o3d.camera.PinholeCameraIntrinsic(
            rgb_img.shape[0], rgb_img.shape[1], cam_K[0, 0], cam_K[1, 1], cam_K[0, 2], cam_K[1, 2]
        )
    ...
    def scene_load(self, scenes_path, scene_num, image_num):
        ...
        try:
            self._scene.scene.add_geometry(
                "annotation_scene", geometry, self.settings.scene_material, add_downsampled_copy_for_fast_rendering=True
            )
            ...
            scene_gt_path = os.path.join(
                self.scenes.scenes_path, f"{self._annotation_scene.scene_num:06}", "scene_gt.json"
            )
...
def main():
    ...
    if os.path.exists(scenes.scenes_path) and os.path.exists(scenes.objects_path):
        ...
        w.window.show_message_box(
            "Error", "Could not find scenes or object meshes folders " + scenes.scenes_path + "/" + scenes.objects_path
        )
